...
let check_color from_p to_p =
  ...
  if is_a_piece to_p
  then (
    if piece_color from_p = piece_color to_p
    then Printf.printf "can not take my piece\n%!";
    false)
  else true
;;
...
let check_rook_move from_x from_y to_x to_y pos =
  ...
  then Printf.printf "rooks only on lines!%!\n"
  else if from_x = to_x
  then (
    ...
    let rec check_path ypath =
      if ypath = to_y
      then true
      else if pos.board.(ypath).(from_x) <> '.'
      then false
      else check_path (ypath + incr)
    in
    let valid = check_path (from_y + incr) in
    if valid
    then Printf.printf "valid rook move\n%!"
    ...
    let rec check_path xpath =
      if xpath = to_x
      then true
      else if pos.board.(from_y).(xpath) <> '.'
      then false
...
let rec gen_pos_list n acc board color last_mv moves =
  match moves with
  | [] ->
    let c = { board; mv_next = None; mv_last = last_mv; active_player = color } in
    List.append acc [ c ]
  | mv :: tail ->
    let c = { board; mv_next = Some mv; mv_last = last_mv; active_player = color } in
    let next_board = copy_board board in
    next_board.(mv.to_x).(mv.to_y) <- get_p next_board mv;
    next_board.(mv.from_x).(mv.from_y) <- '.';
    gen_pos_list (n + 1) (List.append acc [ c ]) next_board (swap color) (Some mv) tail
;;
...
let print_pos pos =
  ...
  (match pos.mv_next with
   | Some m ->
     Printf.printf
       "expect fromx:%i fromy:%i tox:%i toy:%i\n"
       m.from_x
       m.from_y
       m.to_x
       m.to_y
  ...
  (match pos.mv_last with
   | Some m ->
     Printf.printf
       "last mv fromx:%i fromy:%i tox:%i toy:%i\n"
       m.from_x
       m.from_y
       m.to_x
       m.to_y
...
let reverse_position p =
  let nboard =
    Array.fold_left
      (fun acc l ->
        let nl = Array.fold_left (fun acc ch -> Array.append [| ch |] acc) [||] l in
        Array.append [| nl |] acc)
      [||]
      p.board
  in
