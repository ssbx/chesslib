...
let rec list_count_themes chan tht ght prog n =
  ...
  | line ->
    ...
    (match Hashtbl.find_opt ght new_tg with
...
let rec next_nth_line chan i =
  ...
  | 0 ->
    ...
    (match !current_theme with
     ...
     | Theme th ->
       let themes =
         String.split_on_char ' ' (List.nth (String.split_on_char ',' line) 7)
       in
       if List.exists (fun kw -> String.equal th kw) themes
       then line
  ...
  try
    ...
    | id :: fen :: mvs :: rating :: _ ->
      ...
      let player_color, positions =
        ...
        | "w" ->
          ( Types.Black
          , Utils.reverse_positions (Utils.positions_from_mvs fen_pos mvs Types.White) )
