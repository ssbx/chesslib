...
let pm m =
  Array.iter
    (fun a ->
      Array.iter (fun v -> if v then pp " o " else pp " . ") a;
      pp "\n")
    m;
  p "end"
;;
...
let gen_move_matrix mv_mat src_x src_y =
  ...
  Array.iteri (fun i v -> Array.blit mv_mat.(move_orig_y + i) move_orig_x v 0 8) mat;
  mat
;;
...
let get_valid_moves board col src_x src_y = function
  ...
  | 'P' | 'p' ->
    ...
    (* front *)
    if board.(src_y - 1).(src_x) = '.'
    then (
      moves.(src_y - 1).(src_x) <- true;
      (* starting square *)
      if src_y = 6
      then if board.(src_y - 2).(src_x) = '.' then moves.(src_y - 2).(src_x) <- true);
    (* front left *)
    if src_x > 0
    then (
      let sl = board.(src_y - 1).(src_x - 1) in
      if sl <> '.'
      then if Utils.piece_color sl <> col then moves.(src_y - 1).(src_x - 1) <- true);
    (* front right *)
    if src_x < 7
    then (
      let sr = board.(src_y - 1).(src_x + 1) in
      if sr <> '.'
      then if Utils.piece_color sr <> col then moves.(src_y - 1).(src_x + 1) <- true);
    (* en passant todo *)
    moves
...
let check_dest_color c =
  if c.dst_piece <> '.'
  then
    if Utils.piece_color c.src_piece = Utils.piece_color c.dst_piece
    then raise (Wrong_move "src and dst colo are the same")
;;

let check_active_player c =
  if Utils.piece_color c.src_piece <> c.position.active_player
  then raise (Wrong_move "not an active player color piece")
;;
...
let check_piece_move c =
  let mvs =
    get_valid_moves c.position.board c.position.active_player c.src_x c.src_y c.src_piece
  in
  if mvs.(c.dst_y).(c.dst_x) <> true
  then (
