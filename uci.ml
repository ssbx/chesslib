...
let handle_line line =
  if String.equal line "uciok"
  then (
    Printf.printf "--> %s\n" line;
    send "isready")
  else if String.equal line "readyok"
  then
    if !test_fen_send
    then (
      Printf.printf "--> %s\n" line;
...
let update () = consume_buffer ()

let rec loop () =
  (* should use the game loop *)
  if !test_quit <> true
  then (
    update ();
    Unix.sleepf 0.2;
...
let quit () =
  ...
  | p, WEXITED 0 when p = pid ->
    ...
    Unix.close !fd_to_engine;
    Unix.close !fd_from_engine;
    exit 0
